...
#                        [17: LD.ref] 
#                        [18: pleiotropy.file]"
#                        [19: e1.sum_stats.file] 
#                        [20: e2.sum_stats.file] 
#                        [21: e3.sum_stats.file] 
#                        [22: e4.sum_stats.file] 
#                        [23: e5.sum_stats.file] 
#                        [24: e6.sum_stats.file] 
#                        [25: e7.sum_stats.file] 
#                        [26: o.sum_stats.file] 
#                        [27: output.file]"
#                        [28: e1.scale.parameter]"
#                        [29: e2.scale.parameter]"
#                        [30: e3.scale.parameter]"
#                        [31: e4.scale.parameter]"
#                        [32: e5.scale.parameter]"
#                        [33: e6.scale.parameter]"
#                        [34: e7.scale.parameter]"
...
#data preparation  
  #step 1: prepare the initial IVs list
    ...
    e7_uvdat <- read.table(paste(args[15], "_", args[3], "/", args[16], sep = ""), header = T, sep = "\t", stringsAsFactors = FALSE)
    e7_uvdat_tmp <- e7_uvdat[, c("SNP", "EA", "NEA", "BETA", "SE", "P")]
    print(paste("step 1: prepare the exposure7", args[15], "data, input", nrow(e7_uvdat_tmp), "SNPs", ", the dim of exposure7 dataset is", dim(e7_uvdat_tmp)[1], dim(e7_uvdat_tmp)[2]))

    e_uvdat <- rbind(e7_uvdat_tmp, e1_uvdat_tmp, e2_uvdat_tmp, e3_uvdat_tmp, e4_uvdat_tmp, e5_uvdat_tmp, e6_uvdat_tmp)
    #remove duplicated SNPs
    e_uvdat <- e_uvdat[!duplicated(e_uvdat$SNP), ]
    print(paste("step 1: prepare the initial IVs list from exposure1...7,", " total ", nrow(e_uvdat), " SNPs", ", the dim of e_uvdat dataset is", dim(e_uvdat)[1], dim(e_uvdat)[2]))
  ##

  #step 2: select independent (no LD) variants 
    ld_list <- subset(e_uvdat)
    ld_list[which(ld_list$SNP %in% e7_uvdat_tmp$SNP), ]$P <- 0
    names(ld_list)[ncol(ld_list)] <- "pval.exposure"

    e_uvdat_list <- clump_data(ld_list, clump_kb = 1000, clump_r2 = 0.01, clump_p1 = 1, clump_p2 = 1, pop = args[17])    
  ...
  #step 3: remove the pleiotropic snps
    list_func <- function(var = args[15], dat = dat_pleio){
        if (var %in% c("RBC", "Hb", "Ht"))  dat1 <- dat[!(dat$Exposure %in% c(args[2], args[5], args[7], args[9], args[11], args[13], "RBC", "MCV", "MCH", "MCHC", "Hb", "Ht")), ]
        else if (var %in% c("UA")) dat1 <- dat[!(dat$Exposure %in% c(args[2], args[5], args[7], args[9], args[11], args[13], "UA")), ]
        return(dat1)
    }
    
    dat_pleio <- read.table(paste(args[1], "/", args[18], sep = ""), header = T)
    print(paste("the dim of pleiotropy snps is", dim(dat_pleio)[1], dim(dat_pleio)[2]))
    dat_pleio1 <- list_func(var = args[15], dat = dat_pleio)
  ...
  #step 4: select variants existed in the exposures' GWAS dataset
    #exposure 1
      e1_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[19], sep = "\t", snp_col = "SNP",
      ...
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e1_mvdat$beta.outcome <- e1_mvdat$beta.outcome / as.numeric(args[28])
          e1_mvdat$se.outcome   <- e1_mvdat$se.outcome   / as.numeric(args[28])
        } else {
    ...
    #exposure 2
      e2_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[20], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")   
      e2_mvdat$outcome <- args[5]
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e2_mvdat$beta.outcome <- e2_mvdat$beta.outcome / as.numeric(args[29])
          e2_mvdat$se.outcome   <- e2_mvdat$se.outcome   / as.numeric(args[29])
        } else {
    ...
    #exposure 3
      e3_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[21], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")   
      e3_mvdat$outcome <- args[7]
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e3_mvdat$beta.outcome <- e3_mvdat$beta.outcome / as.numeric(args[30])
          e3_mvdat$se.outcome   <- e3_mvdat$se.outcome   / as.numeric(args[30])
        } else {
    ...
    #exposure 4
      e4_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[22], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")   
      e4_mvdat$outcome <- args[9]
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e4_mvdat$beta.outcome <- e4_mvdat$beta.outcome / as.numeric(args[31])
          e4_mvdat$se.outcome   <- e4_mvdat$se.outcome   / as.numeric(args[31])
        } else {
    ...
    #exposure 5
      e5_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[23], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")   
      e5_mvdat$outcome <- args[11]
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e5_mvdat$beta.outcome <- e5_mvdat$beta.outcome / as.numeric(args[32])
          e5_mvdat$se.outcome   <- e5_mvdat$se.outcome   / as.numeric(args[32])
        } else {
    ...
    #exposure 6
      e6_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[24], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")
      e6_mvdat$outcome <- args[13]
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e6_mvdat$beta.outcome <- e6_mvdat$beta.outcome / as.numeric(args[33])
          e6_mvdat$se.outcome   <- e6_mvdat$se.outcome   / as.numeric(args[33])
        } else {
          e6_mvdat$beta.outcome <- e6_mvdat$beta.outcome
          e6_mvdat$se.outcome   <- e6_mvdat$se.outcome  
        }
      ##         
    #leave SNPs
    ...
    #exposure 7: primary exposure 
      e7_mvdat <- read_outcome_data(snps = unique(e_uvdat2$SNP), filename = args[25], sep = "\t", snp_col = "SNP",
                                    beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")
      e7_mvdat$outcome <- args[15]
      names(e7_mvdat) <- gsub("outcome", "exposure", names(e7_mvdat))
      #rescaled by the SDUKB/SDBBJ
        if (args[17] == "EAS") {
          e7_mvdat$beta.exposure <- e7_mvdat$beta.exposure / as.numeric(args[34])
          e7_mvdat$se.exposure   <- e7_mvdat$se.exposure   / as.numeric(args[34])
        } else {
          e7_mvdat$beta.exposure <- e7_mvdat$beta.exposure
          e7_mvdat$se.exposure   <- e7_mvdat$se.exposure  
        }
      ##   

      #orienta to the positive direction for the primary exposure: as suggested by Andrew J. Grant
        e7_mvdat$effect_allele.exposure_c <- e7_mvdat$effect_allele.exposure; e7_mvdat$other_allele.exposure_c <- e7_mvdat$other_allele.exposure
        e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "effect_allele.exposure"] <-   e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "other_allele.exposure_c"]      #effect allele
        e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "other_allele.exposure"]  <-   e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "effect_allele.exposure_c"]      #non effect allele
        e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "eaf.exposure"]           <-   1 - e7_mvdat[ which(e7_mvdat$beta.exposure < 0), "eaf.exposure"]      #effect allele frequency
        e7_mvdat$beta.exposure <- abs(e7_mvdat$beta.exposure)  
        e7_mvdat <- e7_mvdat[, c(1:12)] # SNP EA NEA EAF BETA SE P...... need to check the EAF colunm before
    #leave SNPs
    print(paste("step 4: extract the pooled SNPs from exposure 7's GWAS summary dataset, obtained", nrow(e7_mvdat), "SNPs", ", the dim of e7_mvdat dataset is", dim(e7_mvdat)[1], dim(e7_mvdat)[2]))
    
    #intersection: keep the shared variants among these exposures
      inter_list <- Reduce(intersect, list(e1_mvdat$SNP, e2_mvdat$SNP, e3_mvdat$SNP, e4_mvdat$SNP, e5_mvdat$SNP, e6_mvdat$SNP, e7_mvdat$SNP))
    ##
    print(paste("step 4: extract the shared SNPs from exposure1...7's GWAS summary dataset, totally obtained", length(inter_list), "SNPs"))
  ##

  #step 5: harmonises snps to be all on the same strand
    e1_e6_mvdat <- rbind(subset(e1_mvdat, SNP %in% inter_list), subset(e2_mvdat, SNP %in% inter_list), subset(e3_mvdat, SNP %in% inter_list), 
                         subset(e4_mvdat, SNP %in% inter_list), subset(e5_mvdat, SNP %in% inter_list), subset(e6_mvdat, SNP %in% inter_list))

	  e_mvdat_h <- harmonise_data(e7_mvdat, e1_e6_mvdat, action = 1)
    print(paste("step 5: after harmonises snps (exposure 1 & .. & 6) to be all on the same strand, leaving", length(unique(e_mvdat_h$SNP)), "SNPs", ", the dim of e_mvdat_h dataset is", dim(e_mvdat_h)[1], dim(e_mvdat_h)[2]))
  ##
  ...
  #step 6: format the exposures' dataset
    ...
    e_mvdat_h_qc[e_mvdat_h_qc$exposure == args[15], ]$id.exposure <- args[15]

    e_mvdat_h_qc$id.exposure <- factor(e_mvdat_h_qc$id.exposure, levels = c(args[2], args[5], args[7], args[9], args[11], args[13], args[15]), labels = c(args[2], args[5], args[7], args[9], args[11], args[13], args[15]))
    print(paste("step 6: after format exposures (exposure 1 & .. & 7) for subsequently analysis, leaving", length(unique(e_mvdat_h_qc$SNP)), "SNPs", ", the dim of e_mvdat_h_qc dataset is", dim(e_mvdat_h_qc)[1], dim(e_mvdat_h_qc)[2]))
  ##

  #step 7: select the variants existed in the outcome' GWAS dataset 
    outcome_mvdat <- read_outcome_data(snps = unique(e_mvdat_h_qc$SNP), filename = args[26], sep = "\t", snp_col = "SNP",
                                       beta_col = "BETA", se_col = "SE", effect_allele_col = "EA", other_allele_col = "NEA", eaf_col = "EAF", pval_col = "P")  
  ...
  #step 9: multivariable MR analysis 
    #based on robustMVMR
      #format data
        ...
        dimnames(rob_dat) <- list(c(1:nrow(rob_dat)), c("SNP", "beta.Y", "sebeta.Y", "beta.e1", "beta.e2", "beta.e3", "beta.e4", "beta.e5", "beta.e6", "beta.e7",  
                                                                                     "sebeta.e1", "sebeta.e2", "sebeta.e3", "sebeta.e4", "sebeta.e5", "sebeta.e6", "sebeta.e7"))

        pvalue.Y <- 5e-8; pvalue.e1 <- 5e-8; pvalue.e2 <- 5e-8; pvalue.e3 <- 5e-8; pvalue.e4 <- 5e-8; pvalue.e5 <- 5e-8; pvalue.e6 <- 5e-8; pvalue.e7 <- 5e-8
        rob_dat1 <- cbind(rob_dat, pvalue.Y, pvalue.e1, pvalue.e2, pvalue.e3, pvalue.e4, pvalue.e5, pvalue.e6, pvalue.e7)

        betaGY <- rob_dat1[, "beta.Y"]; sebetaGY <- rob_dat1[, "sebeta.Y"]; pvalbetaGY <- rob_dat1[,"pvalue.Y"]
        
        betaGX <- rob_dat1[, c("beta.e1", "beta.e2", "beta.e3", "beta.e4", "beta.e5", "beta.e6", "beta.e7")]; 
        sebetaGX <- rob_dat1[, c("sebeta.e1", "sebeta.e2", "sebeta.e3", "sebeta.e4", "sebeta.e5", "sebeta.e6", "sebeta.e7")]; 
        pvalbetaGX <- rob_dat1[, c("pvalue.e1", "pvalue.e2", "pvalue.e3", "pvalue.e4", "pvalue.e5", "pvalue.e6", "pvalue.e7")]
      ...
      #output all result
          ...
          robmv_res_out1$Exposure <- c(args[2], args[5], args[7], args[9], args[11], args[13], args[15])
        ...
        write.table(robmv_res_out1, args[27], append = TRUE, row.names = F, col.names = T, quote = F, sep = ",")
      #
    ##
    ...
    #conditional F statistics
      ...
      corr <- corr[row.names(corr) %in% c(args[2], args[5], args[7], args[9], args[11], args[13], args[15]), row.names(corr) %in% c(args[2], args[5], args[7], args[9], args[11], args[13], args[15])]
      print(corr)
      
      cov.exposure <- phenocov_mvmr(corr, mvdat2[, c("sebetaX1", "sebetaX2", "sebetaX3", "sebetaX4", "sebetaX5", "sebetaX6", "sebetaX7")])

      f_data <- format_mvmr(BXGs = mvdat2[, c("betaX1", "betaX2", "betaX3", "betaX4", "betaX5", "betaX6", "betaX7")],
                            BYG = mvdat2[, c("betaYG")],
                            seBXGs = mvdat2[, c("sebetaX1", "sebetaX2", "sebetaX3", "sebetaX4", "sebetaX5", "sebetaX6", "sebetaX7")],
                            seBYG = mvdat2[, c("sebetaYG")],
                            RSID = row.names(mvdat2))
      f_condi <- strength_mvmr(r_input = f_data, gencov = cov.exposure)
      write.table(f_condi, args[27], append = TRUE, row.names = F, col.names = T, quote = F, sep = ",")
    ##  
  ##
##

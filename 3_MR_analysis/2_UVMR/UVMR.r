...
#prepare data
  #step 1: prepare the exposure data
    exposure <- read.table(paste(args[5], sep = ""), header = T, sep = "\t", stringsAsFactors = FALSE)
      #rescaled by SDBBJ/SDUKB
        if (args[18] == "EAS"){
            exposure$BETA <- exposure$BETA / as.numeric(args[19])
            exposure$SE <- exposure$SE / as.numeric(args[19])
        } else if (args[18] == "EUR") {
    ...
    #orienta to the positive direction
        ...
        exposure <- exposure[, c(1:9)] # SNP CHR POS EA NEA EAF BETA SE P. need to check the EAF colunm before
    ##
  ...
  #step 3: remove potential pleiotropic SNPs which is genome-wide significant associated (<5E-8) with other risk factors
    list_func <- function(var = "Height", dat = dat_pleio){
      ...
      else if (var %in% c("RBC", "MCV", "MCHC", "MCH", "Hb", "Ht"))  dat1 <- dat[!(dat$Exposure %in% c("RBC", "MCV", "MCHC", "MCH", "Hb", "Ht")), ]
...
#MR-analysis 
  #1.PVE analysis
    ...
    #output
    write.table(PVE_F, paste("result/", args[14], "/",  args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names = T, quote = F, sep = ",")
  ##

  #2.overlap bias
    if ( as.numeric(args[15]) == 1 ) {
    #N, sample size of exposure GWAS; K, number of valid IVs
    tsmr_bias <- bias_TS(exposure = args[2], outcome = args[3], N = as.numeric(args[11]), K = nrow(e_o_dat_clean1), rsq = PVE, 
    ...
    #output
    write.table(tsmr_bias, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names = T, quote = F, sep = ",")
  ##
  ...
  #3.F statistics
    ...
    write.table(F_result, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names =T, quote = F, sep = ",")
  ##
  
  #4.Cochran's Q heteriogeneity test
    het <- mr_heterogeneity(e_o_dat_clean1, method_list=c("mr_ivw"))
    write.table(het, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names =T, quote = F, sep = ",")
  ##
  ...
  #6.BWMR test
    ...
    write.table(BWMR_result2, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names =T, quote = F, sep = ",")
  ...
  #8.MR.Corr2 results
    ...
    write.table(mr_result_1, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names =T, quote = F, sep = ",")
  ##
  ...
  #9.combined all results
    ...
    write.table(mr_result_plot, paste("result/", args[14], "/", args[2], "_to_", args[3], ".csv", sep = ""), append = TRUE, row.names = F, col.names =T, quote = F, sep = ",")
  ##
  ...
  #10.output the plot results
    ...
    #scatter plot
      mr_scatter_plot(mr_result_plot, e_o_dat_clean1)
      ggsave(paste0("result/", args[14], "/", args[2], "_to_", args[3], "_Scatter_plot.pdf"), plot = last_plot(), width = 30, height = 20, units = "cm", device=cairo_pdf)
    ...
    #funnel plot
      ...
      mr_funnel_plot(mr_result_plot, res_single)
      ggsave(paste0("result/", args[14], "/", args[2], "_to_", args[3], "_Funnel_plot.pdf"), plot = last_plot(), width = 20, height = 20, units = "cm", device=cairo_pdf)
  ##
##
